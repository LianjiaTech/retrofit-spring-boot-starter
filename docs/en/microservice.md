...
Users can implement the `ServiceInstanceChooser` interface to complete the service instance selection logic and register it as a Spring Bean. For Spring Cloud applications, the following implementation can be used:
...
[Previous: GraalVM Native Image / AOT Support](aot.md) | [Next: Custom RetrofitClient Annotations](custom-annotation.md)