...
Circuit breaking is disabled by default. Currently **Sentinel** and **Resilience4j** implementations are supported.

```yaml
retrofit:
  degrade:
    # Circuit breaker type, default none means disabled
...
Set `degrade-type=sentinel` to enable, then declare the `@SentinelDegrade` annotation on the relevant interface or method:
...
### Global Sentinel Circuit Breaking
...
Set `degrade-type=resilience4j` to enable, then declare `@Resilience4jDegrade` on the relevant interface or method:
...
### Global Resilience4j Circuit Breaking
...
## Extending Circuit Breaking

If you need to use other circuit breaking implementations, extend `BaseRetrofitDegrade` and register it as a Spring Bean.

## Fallback and FallbackFactory

If `@RetrofitClient` does not set `fallback` or `fallbackFactory`, when circuit breaking is triggered, a `RetrofitBlockException` will be thrown directly. Users can customize the method return value during circuit breaking by setting `fallback` or `fallbackFactory`.

> Note: The `fallback` class must be an implementation class of the current interface, and `fallbackFactory` must be an implementation class of `FallbackFactory<T>` with the generic parameter type being the current interface type. Also, `fallback` and `fallbackFactory` instances must be registered as Spring Beans.

The main difference between `fallbackFactory` and `fallback` is that `fallbackFactory` can感知 the exception cause (cause) of each circuit breaking event.
...
public class HttpDegradeFallbackFactory implements FallbackFactory<HttpDegradeApi> {
    ...
    public HttpDegradeApi create(Throwable cause) {
        log.error("Circuit breaking triggered! ", cause.getMessage(), cause);
