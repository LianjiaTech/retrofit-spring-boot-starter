...
本節では一般的な HTTP リクエストシーンの例を紹介します。
...
public class DownloadTest {
    ...
    public void download() throws Exception {
        ...
        // バイナリストリームの処理はビジネスで制御します。ここではファイル書き込みの例を示します
...
`@Url` アノテーションでダイナミック URL を実現できます。この場合、`baseUrl` は任意の有効な URL を設定し、ランタイムでは `@Url` のアドレスに基づいてリクエストを発行します。

> 注意：`@Url` はメソッドパラメータの最初の位置に配置する必要があります。また、`@GET`、`@POST` 等のアノテーションではエンドポイントパスを定義しないでください。
...
OkHttp 自体は GET リクエストにリクエストボディを追加することをサポートしていません（[OkHttp Issue #3154](https://github.com/square/okhttp/issues/3154) を参照）。本当に必要な場合は、小文字 `get` で制限を回避できます：
...
[前節：全設定項目リファレンス](configuration.md)